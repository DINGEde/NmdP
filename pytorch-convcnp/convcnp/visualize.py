...
def plot_all(xc, yc, xt, yt, model_xt, gp_pred, np_pred, support=False):
    ...
    plt.ylim(-3, 3)
    return plot_model_output(model_xt.cpu(), np_pred.mean.cpu()[0], np_pred.scale.cpu()[0], 'purple')
    #return plot_model_output(model_xt.cpu(), np_pred.mean.cpu()[0], np_pred.stddev.cpu()[0], 'purple')

def plot_all_muti_variate(xc, yc, xt, yt, model_xt, gp_pred, np_pred, support=False):
    gp_y_pred, gp_std = gp_pred
    plt.plot(xc.cpu()[0], yc.cpu()[0], 'o', color='black')
    plt.plot(xt.cpu()[0], yt.cpu()[0], '-', color='black')
    plot_model_output(model_xt.cpu(), gp_y_pred, gp_std, 'green')
